...
describe("snapshot store", () => {
  ...
  it("older REST response cannot replace newer WebSocket snapshot", () => {
    ...
    expect(store.acceptSnapshot(snapshotAt("2026-09-05T10:00:01Z", 10))).toBe(
      false,
    );
  ...
  it("equal generated_at is idempotently accepted", () => {
    ...
    expect(store.acceptSnapshot(snapshotAt("2026-09-05T10:00:02Z", 73))).toBe(
      true,
    );
  ...
  it("one command key does not block another command", () => {
    ...
    expect(store.getState().commandKeys.size).toBe(0);
  });
});
