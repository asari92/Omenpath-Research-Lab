...
import { densityProfile, distance, dot, radialUnit, spawnParticle } from './particles';
...
describe('portal particles', () => {
  it('spawns on the circumference with outward and tangential motion', () => {
    ...
    expect(dot(radialUnit(particle, { x: 88, y: 88 }), particle.velocity)).toBeGreaterThan(0);
    const radial = radialUnit(particle, { x: 88, y: 88 });
    expect(Math.abs(radial.x * particle.velocity.y - radial.y * particle.velocity.x)).toBeGreaterThan(0);
  });
