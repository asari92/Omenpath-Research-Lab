...
describe('PortalActions', () => {
  ...
  it('available action invokes only its exact command', async () => {
    ...
    render(<PortalActions onCommand={onCommand} portalId={42} quickActions={actions} />);
  ...
  it('native-disables only the selected in-flight action', () => {
    render(
      <PortalActions
        busyKey="42:CLOSE"
        portalId={42}
        quickActions={{ ...actions, can_send_observer: true, send_observer_unavailable_reason: null }}
      />,
    );
    expect(screen.getByRole('button', { name: 'Close' })).toBeDisabled();
    expect(screen.getByRole('button', { name: 'Send Observer' })).not.toBeDisabled();
  });
  ...
  it('Empty Slot uses the same four controls with a local reason', async () => {
    ...
    expect(screen.getByRole('status')).toHaveTextContent(/no portal occupies this slot/i);
  });
});
