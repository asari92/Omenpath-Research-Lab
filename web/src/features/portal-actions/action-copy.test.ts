...
describe("unavailable action copy", () => {
  it.each([
    ...
    [
      ...
      "Portal direction conflicts with this movement.",
    ],
    ...
    [
      ...
      "Extraction synchronization is still running.",
    ],
  ])("maps %s", (code, message) => {
    expect(unavailableActionCopy(code)).toBe(message);
  });

  it("safely explains an unknown future code", () => {
    expect(unavailableActionCopy("FUTURE_REASON")).toBe(
      "Action unavailable: FUTURE_REASON",
    );
  });
});
