...
test("long-form Worklog scrolls inside the viewport and Dashboard stays fixed", async ({
  ...
  await article.hover({ position: { x: 40, y: 40 } });
  await page.mouse.wheel(0, 2000);
  await expect
    .poll(() => article.evaluate((node) => node.scrollTop))
    .toBeGreaterThan(100);
  const scroll = await article.evaluate((node) => {
  ...
  await expect(article.locator(":scope > :last-child")).toBeInViewport();
  expect(
    await page.evaluate(
