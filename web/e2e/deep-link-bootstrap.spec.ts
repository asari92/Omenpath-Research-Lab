...
for (const routePath of ["/events", "/portals/1"]) {
  test(`fresh direct ${routePath} recovers failed bootstrap through Retry connection`, async ({
    page,
    context,
  }) => {
    let allowBootstrap = false;
    let resourceRequests = 0;
    let socketSnapshots = 0;
    page.on("request", (request) => {
      if (new URL(request.url()).pathname === `/api${routePath}`)
        resourceRequests++;
    });
    page.on("websocket", (socket) => {
      if (new URL(socket.url()).pathname === "/ws/lab")
        socket.on("framereceived", () => socketSnapshots++);
    });
      ...
      if (!allowBootstrap) {
        await route.fulfill({
          ...
          json: {
            error: {
              code: "UNAVAILABLE",
              message: "Bootstrap temporarily unavailable",
              confirmable: false,
            },
          },
        });
    ...
    expect(socketSnapshots).toBe(0);
    expect(await context.cookies()).toHaveLength(0);
    allowBootstrap = true;
    await retry.click();
    await expect.poll(() => resourceRequests).toBeGreaterThan(0);
    await expect.poll(() => socketSnapshots).toBeGreaterThan(0);
    await expect(page.getByLabel("Connection state")).toContainText(
      "Planar link stable",
    );
    expect(
  ...
  test(`fresh direct ${routePath} establishes one cookie before route REST and WebSocket`, async ({
    page,
    context,
  }) => {
    let release!: () => void;
    ...
    page.on("request", (request) => {
      if (new URL(request.url()).pathname === `/api${routePath}`)
        resourceCookies.push(
          request.allHeaders().then((headers) => headers.cookie),
        );
    });
    page.on("response", (response) => {
      if (new URL(response.url()).pathname.startsWith("/api/"))
        cookieResponses.push(
          response.allHeaders().then((headers) => headers["set-cookie"]),
        );
    });
    page.on("websocket", (socket) => {
    ...
    try {
      ...
      else await expect(page.getByText("Portal Not Found")).toBeVisible();
      const cookies = (await context.cookies()).filter(
        (cookie) => cookie.name === "omenpath_session",
      );
      expect(cookies).toHaveLength(1);
      expect(
        (await Promise.all(cookieResponses)).filter((header) =>
          header?.includes("omenpath_session="),
        ),
      ).toHaveLength(1);
      ...
      expect(
        (await Promise.all(resourceCookies)).every((header) =>
          header?.includes(`omenpath_session=${cookies[0].value}`),
        ),
      ).toBe(true);
      expect(
