...
test('desktop keeps seven stable slots in a 4 + 3 board', async ({ page }) => {
  await page.goto('/');
  await expect(page.getByTestId('portal-slot')).toHaveCount(7);
  const firstRow = await page.getByTestId('portal-slot').evaluateAll((slots) =>
    slots.slice(0, 4).map((slot) => Math.round(slot.getBoundingClientRect().top)),
  );
  expect(new Set(firstRow).size).toBe(1);
});

test('phone keeps all seven slots and four commands per slot without board scroll', async ({ page }) => {
  await page.goto('/');
  await expect(page.getByTestId('portal-slot')).toHaveCount(7);
