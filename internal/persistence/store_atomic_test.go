...
func TestStoreCommit_AssignsEventIDsInChronologicalOrder(t *testing.T) {
	...
	events, err := testLab(t, store).Commit(ctx, snapshot, drafts)
...
func TestStoreListEvents_GlobalAndPortalUseSameRows(t *testing.T) {
	...
	_, err := testLab(t, store).Commit(ctx, snapshot, []domain.EventDraft{
	...
	global, err := testLab(t, store).ListEvents(ctx, nil)
	require.NoError(t, err)
	history, err := testLab(t, store).ListEvents(ctx, &portal7)
	require.NoError(t, err)
...
func TestStoreCommit_RollsBackStateWhenEventInsertFails(t *testing.T) {
	...
	_, err = testLab(t, store).Commit(ctx, after, []domain.EventDraft{
		failingDraft,
	})
	require.Error(t, err)
...
func TestStoreCommit_ActionRejectedPreservesMissingRequestedEntityIDs(t *testing.T) {
	...
	persisted, err := testLab(t, store).Commit(ctx, snapshot, []domain.EventDraft{draft})
	require.NoError(t, err)
...
func TestStoreListEvents_RejectsCorruptPayloadJSON(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = testLab(t, store).ListEvents(ctx, nil)
	require.Error(t, err)
}
