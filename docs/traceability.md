...
| ID | Rule | Type | Test | Implementation | Status | Notes |
|---|---|---|---|---|---|---|
| EVENT-001 | Meaningful transition → Event | BEH | `TestEventsForTransition_CloseAndCollapse`, `TestEventsForTransition_OutboundArrivalStartsResearch`, `TestEventsForTransition_ReturnAndLoss`, `TestEventsForTransition_ExtractionSyncBeforeAutomaticReturn`, `TestSimulationTick_EventOrderUsesSemanticTime` | `EventsForStateTransition`, `SimulationState.ResolveTick` | PARTIAL | deterministic simulation and generic before/after domain transitions emit drafts; Stage 11 command orchestration remains |
...
| EVENT-003 | Rejected action → ACTION_REJECTED | BEH | `TestNewActionRejectedEvent_EncodesDomainCause` | `NewActionRejectedEvent` | PARTIAL | validated draft factory exists; Stage 11 must invoke it for every domain command rejection |
...
| EVENT-007 | Прибытие в Plane создаёт arrived + research started | BEH | `TestEventsForTransition_OutboundArrivalStartsResearch`, `TestEventsForTransition_LateTickReconstructsObserverPhases`, `TestEventsForTransition_ReturnAndLoss` | `EventsForStateTransition` | GREEN | late resolution reconstructs arrival, research and return semantic timestamps |
...
| EVENT-010 | Граница domain rejection | BEH | `TestNewActionRejectedEvent_EncodesDomainCause` | `NewActionRejectedEvent` | PARTIAL | pure domain rejection draft is covered; Stage 11/12 must distinguish domain actions from malformed transport/routes |
