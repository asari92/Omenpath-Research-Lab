...
| ID | Rule | Type | Test | Implementation | Status | Notes |
|---|---|---|---|---|---|---|
| SIMULATION-001 | One-second supplied-time domain step | BEH | `TestSimulationState_InvalidAggregateIsAtomic`, `TestSimulationTick_ResolvesNaturalClose`, `TestSimulationTick_OutboundAtDeadlineStartsResearch`, `TestSimulationTick_ExtractionAtSyncCompletes`, `TestSimulationTick_ResultReportsCurrentLabEnergy` | `SimulationState.ResolveTick` | PARTIAL | complete pure supplied-time Stage 8 transition step exists; recurring ticker ownership remains Stage 11 |
| SIMULATION-002 | Ordered tick transition stages | BEH | `TestSimulationTick_TerminalPortalReleasesSlotBeforeNaturalStage`, `TestSimulationTick_MultipleCollapsesIgnorePortalSliceOrder`, `TestSimulationTick_PortalLifecycleRunsBeforeObserverLifecycle`, `TestSimulationTick_ObserversAreVisitedByID`, `TestSimulationTick_ResearchCompletionFeedsSameTickExtraction`, `TestSimulationTick_ExtractionDrawsTransitDurationBeforeNaturalSpawn` | `resolvePortalStage`, `resolveObserverStage`, `resolveExtractionStage`, `resolveNaturalSpawnPrepared` | PARTIAL | Portal → Observer-by-ID → Extraction-by-ID → Natural order is implemented; post-spawn Attention integration is finalized in checkpoint H |
| SIMULATION-003 | Atomic, monotonic, idempotent tick | INV | `TestSimulationState_InvalidAggregateDoesNotConsumeRandom`, `TestSimulationState_InvalidAggregateIsAtomic`, `TestSimulationTick_PortalStageFailureIsAtomic`, `TestSimulationTick_PortalStageFailureConsumesNoRandom`, `TestSimulationTick_ObserverStageFailureIsAtomic`, `TestSimulationTick_InvalidExtractionAggregateIsAtomic` | `validateSimulationState`, `SimulationState.ResolveTick`, `resolvePortalStage`, `resolveObserverStage`, `resolveExtractionStage` | PARTIAL | validation and Portal/Observer/Extraction failures are atomic; full-stage monotonic/idempotent regressions remain checkpoint H |
| SIMULATION-004 | Realtime values stay derived | INV | `TestSimulationTick_DerivedPortalEnergyIsNotPersisted`, `TestSimulationTick_DerivedCreaturesAreNotPersisted`, `TestSimulationTick_DerivedLabEnergyIsNotRebased`, `TestSimulationTick_ResultReportsCurrentLabEnergy` | `SimulationState.ResolveTick`, `deriveSimulationTickResult` | GREEN | Portal Energy, creatures and Lab Energy remain derived from stored baselines; tick result derives current Lab Energy after committed transitions |
