...
| ID | Rule | Type | Test | Implementation | Status | Notes |
|---|---|---|---|---|---|---|
| SIMULATION-001 | One-second supplied-time domain step | BEH | `TestSimulationState_InvalidAggregateIsAtomic`, `TestSimulationTick_ResolvesNaturalClose`, `TestSimulationTick_OutboundAtDeadlineStartsResearch`, `TestSimulationTick_ResultReportsCurrentLabEnergy` | `SimulationState.ResolveTick` | PARTIAL | supplied-time tick now runs Portal, Observer and Natural stages; Extraction remains checkpoint G and real ticker remains Stage 11 |
| SIMULATION-002 | Ordered tick transition stages | BEH | `TestSimulationTick_TerminalPortalReleasesSlotBeforeNaturalStage`, `TestSimulationTick_MultipleCollapsesIgnorePortalSliceOrder`, `TestSimulationTick_PortalLifecycleRunsBeforeObserverLifecycle`, `TestSimulationTick_ObserversAreVisitedByID` | `resolvePortalStage`, `resolveObserverStage`, `resolveNaturalSpawnPrepared` | PARTIAL | Portal lifecycle precedes ID-ordered Observer catch-up, which precedes Natural generation; Extraction is added in checkpoint G |
| SIMULATION-003 | Atomic, monotonic, idempotent tick | INV | `TestSimulationState_InvalidAggregateDoesNotConsumeRandom`, `TestSimulationState_InvalidAggregateIsAtomic`, `TestSimulationTick_PortalStageFailureIsAtomic`, `TestSimulationTick_PortalStageFailureConsumesNoRandom`, `TestSimulationTick_ObserverStageFailureIsAtomic` | `validateSimulationState`, `SimulationState.ResolveTick`, `resolvePortalStage`, `resolveObserverStage` | PARTIAL | validation and Portal/Observer-stage failures are atomic; full-stage monotonic/idempotent regressions remain checkpoint H |
