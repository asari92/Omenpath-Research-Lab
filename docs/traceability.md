...
| ID | Rule | Type | Test | Implementation | Status | Notes |
|---|---|---|---|---|---|---|
| SIMULATION-001 | One-second supplied-time domain step | BEH | `TestSimulationState_InvalidAggregateIsAtomic`, `TestSimulationTick_ResolvesNaturalClose`, `TestSimulationTick_ResultReportsCurrentLabEnergy` | `SimulationState.ResolveTick` | PARTIAL | pure supplied-time tick now runs Portal lifecycle and Natural generation; Observer/Extraction stages remain checkpoints F/G and real ticker remains Stage 11 |
| SIMULATION-002 | Ordered tick transition stages | BEH | `TestSimulationTick_TerminalPortalReleasesSlotBeforeNaturalStage`, `TestSimulationTick_MultipleCollapsesIgnorePortalSliceOrder` | `resolvePortalStage`, `resolveNaturalSpawnPrepared` | PARTIAL | Portal lifecycle precedes Natural generation; Observer and Extraction stages are added in checkpoints F/G |
| SIMULATION-003 | Atomic, monotonic, idempotent tick | INV | `TestSimulationState_InvalidAggregateDoesNotConsumeRandom`, `TestSimulationState_InvalidAggregateIsAtomic`, `TestSimulationTick_PortalStageFailureIsAtomic`, `TestSimulationTick_PortalStageFailureConsumesNoRandom` | `validateSimulationState`, `SimulationState.ResolveTick`, `resolvePortalStage` | PARTIAL | validation and Portal-stage failures are atomic and draw-free; full-stage regressions remain checkpoint H |
